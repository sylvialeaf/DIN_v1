...
class SASRec(nn.Module):
    ...
    def get_attention_mask(self, item_seq):
        ...
        # 转换为加法掩码：True -> 0, False -> -10000
        extended_attention_mask = torch.where(
            extended_attention_mask.bool(), 
            torch.tensor(0.0, device=item_seq.device), 
            torch.tensor(-10000.0, device=item_seq.device)
        )
...
class NARM(nn.Module):
    ...
    def forward(self, batch):
        ...
        gru_output, _ = nn.utils.rnn.pad_packed_sequence(
            packed_output, 
            batch_first=True,
            total_length=item_seq.shape[1]
        )  # [B, L, H]
        
        # 全局表示: 最后隐藏状态
        h_global = hidden[-1]  # [B, H]
        ...
        attention_scores = attention_scores.masked_fill(~seq_mask.bool(), -1e9)
