...
parser.add_argument('--topk-sample', type=int, default=None,
                    help='Top-K 评估采样用户数（默认全部，设置后可加速）')
...
TOPK_SAMPLE_USERS = args.topk_sample  # None 表示全部用户
...
if args.quick:
    ...
    EPOCHS = args.epochs
    SEQ_LENGTHS = [150]
    # RTX 3080 Ti 12GB 显存，可以用较大的 batch size
    # 多 GPU 时 batch size 可以翻倍
    BASE_BATCH_SIZE = 4096 if DEVICE == 'cuda' else 256
    BATCH_SIZE = BASE_BATCH_SIZE * NUM_GPUS if USE_MULTI_GPU else BASE_BATCH_SIZE
...
# 增加 workers 数量以提高数据加载速度
NUM_WORKERS = min(12, CPU_COUNT - 2) if DEVICE == 'cuda' else 0

# 预取因子：每个 worker 预加载的 batch 数
PREFETCH_FACTOR = 4  # 默认是 2，增加可以减少 GPU 等待

MODELS_TO_TEST = ['SASRec']
...
if TOPK_SAMPLE_USERS:
    print(f"Top-K 采样用户数: {TOPK_SAMPLE_USERS}")
...
def run_experiment1(dataset_name):
    ...
    for seq_length in SEQ_LENGTHS:
        ...
        if ENABLE_TOPK:
            ...
            # 采样用户（加速）
            if TOPK_SAMPLE_USERS and len(eval_data) > TOPK_SAMPLE_USERS:
                import random
                random.seed(2020)
                eval_data = random.sample(eval_data, TOPK_SAMPLE_USERS)
                print(f"  (Top-K 采样 {TOPK_SAMPLE_USERS} 用户)")
        ...
        for model_name in MODELS_TO_TEST:
            ...
            try:
                ...
                trainer = RichTrainer(model=model, device=DEVICE, use_multi_gpu=USE_MULTI_GPU)
                t1 = time.time()
...
def run_experiment2(dataset_name):
    ...
    if HAS_LIGHTGBM and din_model is not None:
        ...
        try:
            ...
            topk_metrics = {}
            if ENABLE_TOPK and eval_data is not None:
                # 需要用 hybrid 预测分数，构造 Top-K 评估
                # 复用 trainer.evaluate_topk 的流程，但用 hybrid 预测分数
                from tqdm import tqdm
                all_labels = []
                all_scores = []
                for entry in tqdm(eval_data, desc="Hybrid Top-K"):
                    user_feat = fp_eval.process_user(entry['user_id'])
                    seq_feat = fp_eval.process_seq(entry['hist_items'], entry['hist_genres'], entry['hist_years'])
                    item_feats = [fp_eval.process_item(i) for i in entry['candidates']]
                    batch = fp_eval.collate_eval_batch(user_feat, seq_feat, item_feats)
                    # 用 hybrid 预测分数
                    with torch.no_grad():
                        features, _ = hybrid.extract_din_features([batch])
                        scores = hybrid.lgb_model.predict(features)
                    all_scores.append(scores)
                    all_labels.append(np.array(entry['labels']))
                # 拼接
